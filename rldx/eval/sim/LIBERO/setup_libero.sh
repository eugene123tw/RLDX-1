...
# robosuite==1.4.0 (pulled in by LIBERO) uses the old MjData.qM API, which newer
# mujoco releases renamed/removed (AttributeError: 'MjData' object has no attribute 'qM').
# Pin to a version that still exposes qM.
