...
detect_visible_gpus() {
    if [[ -n "${CUDA_VISIBLE_DEVICES:-}" ]]; then
        # CUDA_VISIBLE_DEVICES="0,1,2" -> 3 visible devices.
    ...
    # Fallback for environments where nvidia-smi is unavailable.
    echo 1
}

DETECTED_GPUS="$(detect_visible_gpus)"
NUM_GPUS="${NUM_GPUS:-$DETECTED_GPUS}"
# Safer default for 1x A100-40GB with video_length=4 and this model size.
# With gradient_checkpointing + bf16, even 2 per_device can be tight; default conservatively.
...
# Optimizer: adafactor is much lighter on optimizer-state memory than AdamW.
OPTIM="${OPTIM:-adafactor}"
# Memory-saving trainable-surface defaults for 1x A100-40GB.
